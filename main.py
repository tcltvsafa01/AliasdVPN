...
def main_menu(user_id):
    buttons = [
        [InlineKeyboardButton("🛒 خرید کانفیگ", callback_data="shop")],
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "home":
        ...
        cur.execute("""
            SELECT id, name, config_type, duration, volume,
                   users, price, stock
            FROM products
        """)
        ...
        for product in products:
            pid, name, ctype, duration, volume, users, price, stock = product
        ...
        await query.edit_message_text(
            "💬 پشتیبانی\n\n"
            "برای ارتباط با پشتیبانی، فعلاً از طریق مدیر فروشگاه اقدام کنید.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 برگشت", callback_data="home")]
            ])
        )
        ...
        cur.execute("""
            SELECT id, name, config_type, duration,
                   volume, users, price, stock
            FROM products
        """)
        ...
        if not products:
            ...
            for p in products:
                pid, name, ctype, duration, volume, users, price, stock = p
        ...
        cur.execute("""
            SELECT name, config_type, duration, volume,
                   users, price, stock
            FROM products
            WHERE id = ?
        """, (product_id,))
        ...
        name, ctype, duration, volume, users, price, stock = product
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 محصولات", callback_data="shop")]
            ])
        )
